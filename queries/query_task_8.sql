...
SELECT COUNT(j.subscriber_first_name) FROM (SELECT s.subscriber_id, s.subscriber_first_name, s.subscriber_address_street, sf.subscriber_intercity_access FROM subscribers s JOIN subscriber_fees sf ON s.subscriber_id = sf.subscriber_id GROUP BY s.subscriber_id) j WHERE j.subscriber_address_street = '97 Florence Lane' AND j.subscriber_intercity_access = true;

-- SELECT COUNT(subscriber_id) FROM subscribers WHERE subscriber_id IN (SELECT station_id FROM stations WHERE station_type = 'municipal');

SELECT COUNT(j.subscriber_first_name) FROM (SELECT s.subscriber_id, s.subscriber_first_name, s.subscriber_address_street, sf.subscriber_intercity_access FROM subscribers s JOIN subscriber_fees sf ON s.subscriber_id = sf.subscriber_id GROUP BY s.subscriber_id) j WHERE j.subscriber_address_street LIKE '%Florence Lane' AND j.subscriber_intercity_access = true;
